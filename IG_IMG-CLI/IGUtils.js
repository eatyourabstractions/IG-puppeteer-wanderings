...
async function logMeIn(mypage, username, pass){
    ...
    await mypage.type('input[name="username"]', '@' + username);
...
async function listFollowers(myPage, who){
    ...
    async function mapOverPics(myPage, aProfile, asyncFunc){
      ...
      async function loop(array, asyncFunc) {
        results = []
        let index = 0;
        function next() {
            if (index < array.length) {
      ...
      function downloadAsync(TheName){
        async function returnMe(elem, idx){
          return request(elem).pipe(fs.createWriteStream(TheName + ''+ idx +'.png'))
        }
      ...
      async function begin(){
        const browserHandle = await puppeteer.launch({headless: true, slowMo: 100});
        const page = await browserHandle.newPage();
        return {tab: page, browser: browserHandle}

      }

      function checkDirectory(dir = "picDB"){
